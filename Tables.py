...
#import filedialog, tkMessageBox, tkSimpleDialog
...
class TableCanvas(Canvas):
    ...
    def set_defaults(self):
        ...
        self.y_start=1
        self.linewidth=1.0
        self.rowheaderwidth=150
        self.showkeynamesinheader=True
    ...
    def createTableFrame(self, callback=None):
        ...
        self.tablerowheader = RowHeader(self.parentframe, self, width=150)
        self.tablecolheader = ColumnHeader(self.parentframe, self,bg="gray")
        ...
        self.tablerowcolumnheader = Label(self.parentframe,font=self.thefont,text="Properties",background="gray",foreground="white")
        self.tablerowcolumnheader.grid(row=0,column=0,rowspan=1,sticky='news')
        ...
        self.grid(row=1,column=1,rowspan=1,sticky='news')
    ...
    def redrawVisible(self, event=None, callback=None):
        ...
        self.visiblerows = range(int(startvisiblerow), int(endvisiblerow))
        startvisiblecol, endvisiblecol = self.getVisibleCols(x1, x2)
        self.visiblecols = range(int(startvisiblecol), int(endvisiblecol))
    ...
    def select_All(self):
        ...
        self.multiplerowlist = range(int(self.startrow),int(self.endrow))
        self.drawMultipleRows(self.multiplerowlist)
        self.startcol = 0
        self.endcol = self.cols
        self.multiplecollist = range(int(self.startcol), int(self.endcol))
        self.drawMultipleCells()
        return
    ...
    def handle_mouse_drag(self, event):
        ...
        if colover > self.cols or self.startcol > self.cols:
            ...
            if self.endcol < self.startcol:
                self.multiplecollist=range(int(self.endcol), int(self.startcol+1))
            else:
                self.multiplecollist=range(int(self.startcol), int(self.endcol+1))
            #print self.multiplecollist
        #draw the selected rows
        if self.endrow != self.startrow:
            if self.endrow < self.startrow:
                self.multiplerowlist=range(int(self.endrow), int(self.startrow+1))
            else:
                self.multiplerowlist=range(int(self.startrow), int(self.endrow+1))
            self.drawMultipleRows(self.multiplerowlist)
            self.tablerowheader.drawSelectedRows(self.multiplerowlist)
    ...
    def checkOSType(cls):
        ...
        var_s=['OSTYPE','OS']
        for var in var_s:
            #if os.environ.has_key(var): python 2.7
            if var in os.environ:
                #ostyp=string.lower(os.environ[var]) python 2.7
...
class ColumnHeader(Canvas):
    ...
    def __init__(self, parent=None, table=None,bg='gray25'):
        Canvas.__init__(self, parent, bg=bg, width=500, height=20,bd=0)
    ...
    def redraw(self):
        ...
        for col in self.table.visiblecols:
            ...
            if len(collabel)>w/10:
                collabel=collabel[0:int(w/12)]+'.' #RM# Width(w) changing to non int 
    ...
    def handle_left_shift_click(self, event):
        ...
        if colclicked > currcol:
            self.table.multiplecollist = range(int(currcol), int(colclicked+1))
        elif colclicked < currcol:
            self.table.multiplecollist = range(int(colclicked), int(currcol+1))
        else:
            return
...
class RowHeader(Canvas):
    """Class that displays the row headings on the table
       ...
       selection"""
    def __init__(self, parent=None, table=None, width=150):
        Canvas.__init__(self, parent, bg='gray75', width=width, height=None,highlightthickness=0,bd=0)
    ...
    def handle_mouse_drag(self, event):
        ...
        if self.endrow != self.startrow:
            if self.endrow < self.startrow:
                print(type(self.endrow))
                print(type(self.startrow))
